//This code is for object detection using both Ultrasonic and IR sensors
//IR - VMA330 : pin 4 

//Ultrasonic : trigpin 2, echopin a7  (using interrupts)
...
void setup() {
  ...
  //Timer0.attachInterrupt(getUltrasonicValue);
  
  Serial.begin(115200);

  Timer.getAvailable().attachInterrupt(getUltrasonicValue).start();delay(50);
...
void loop() {
  //Serial.println(getIrValue() + " and " + getUltrasonicValue());
  while(1){
    //Serial.println("hi");
    }
}
...
void getUltrasonicValue(){
//  if (CaptureFlag){
//    CaptureFlag = 0;   //reset the flag 
//    int UsSensorValue = 340.0*CaptureCountA/(42000000.0)/2*100;
//    return UsSensorValue;
//  }
  Serial.println(usSensor.getDistCM()); //returns the distance in cm 
}
