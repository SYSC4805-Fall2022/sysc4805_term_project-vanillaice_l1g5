...
void UltrasonicHandler() {
  ...
  if (US_DIST_RIGHT < ULTRASONIC_THRESH && US_DIST_RIGHT > 0.0 && !US_FLAG_RIGHT) {
    US_FLAG_RIGHT = 1;  //Set flag for object avoidance subroutine
  ...
  if (US_DIST_LEFT < ULTRASONIC_THRESH && US_DIST_LEFT > 0.0 && !US_FLAG_LEFT) {
    US_FLAG_LEFT = 1;  //Set flag for object avoidance subroutine
...
void avoidObject() {
  ...
  if (US_FLAG_RIGHT) {
    ...
    moveForwardTimed();
    printf("done right\n");
    US_FLAG_RIGHT = 0; // Reset flag when done handling object avoidance
    ...
    US_FLAG_LEFT = 0; // Reset flag when done handling object avoidance
